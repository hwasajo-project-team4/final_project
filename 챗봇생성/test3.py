...
def content_based_recommendation_with_weights(age, gender, skin_type, skin_trouble, top_n=5, weight=0.1, df=df):
...
def recommend_products_for_user(age, gender, skin_type, skin_trouble, top_n=5, df=df):
    # 가상 유저 ID를 생성
    ...
    if user_review_count <= heavy_user_threshold:
        return content_based_recommendation_with_weights(age, gender, skin_type, skin_trouble, top_n=top_n, df=df)
    else:
        user_id = user_to_id[virtual_user]
        # CF 기반 추천 수행
        user_recommendations_with_rated = get_top_n_recommendations(df, n=5)
