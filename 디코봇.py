...
# 사용자 정보와 일치하는 리뷰 데이터 필터링
    filtered_df = skincare[(skincare['나이'] == age) & (skincare['성별'] == gender) &
                    (skincare['피부타입'] == skin_type) & (skincare['피부트러블'] == skin_trouble)]
...
class MyClient(discord.Client):
    ...
    async def recommend_products_for_user(self, skincare, message, top_n=5):
        ...
        if user_review_count <= heavy_user_threshold:
            recommended_products = content_based_recommendation_with_weights(age, gender, skin_type, skin_trouble, top_n=top_n)
        else:
